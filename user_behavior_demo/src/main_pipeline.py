...
def prepare_embeddings(
    ...
    sequence_embeddings = np.array(get_user_embeddings(sequences, w2v_model))

    # Aggregate invoice-level embeddings to customer granularity.
    w2v_df = pd.DataFrame(sequence_embeddings)
    w2v_df["CustomerID"] = sequence_customers
    base_customers = feature_table["CustomerID"].astype(str).tolist()
    customer_embeddings = w2v_df.groupby("CustomerID").mean().reindex(base_customers)
    customer_embeddings = customer_embeddings.fillna(0.0)
    word2vec_embeddings = customer_embeddings.values
...
def run_pipeline(
    ...
    weighted, feature_table, sequences, sequence_customers = preprocess(
        transactions_path=str(transactions_path),
        touchpoints_path=str(touchpoints_path),
        config=config,
    )

    embeddings, tables = prepare_embeddings(
        weighted,
        feature_table,
        sequences,
        sequence_customers,
    ...
    def kmeans_clusterer(features: np.ndarray) -> np.ndarray:
        ...
        # Keep the demo simple by capping the number of clusters while also
        # respecting the constraints expected by downstream metrics (at least
        # 2 clusters and strictly fewer clusters than samples).
        max_clusters = max(2, min(3, n_samples))
        n_clusters = min(max_clusters, max(n_samples - 1, 1))

        # When there are too few samples for meaningful clustering we fall back
        # to assigning everyone to a single segment to avoid runtime errors.
        if n_clusters < 2 or n_samples < 3:
            return np.zeros(n_samples, dtype=int)

        _, labels = run_kmeans(features, n_clusters=n_clusters)
        return labels
