...
def build_user_sequences(df: pd.DataFrame) -> List[List[str]]:
    """Return product sequences ordered by time for Word2Vec training."""

    df_sorted = df.sort_values("InvoiceDate")
    grouped = df_sorted.groupby(["CustomerID", "InvoiceNo"])
    sequences: List[List[str]] = []
    for _, invoice in grouped:
...
def preprocess(
    ...
    sequences = build_user_sequences(weighted)
    feature_table = build_feature_table(weighted, touchpoints)
    return weighted, feature_table, sequences
